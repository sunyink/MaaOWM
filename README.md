...
### 5. MaaOWM 在你的资源项目里留下什么

所有 owm 文件都在**被管理项目根**的 `maaowm/` 下，**适配包目录内不产生任何 owm 文件**：

```
maaowm/
├── .gitignore      自带, 忽略 .state/ —— 你不需要再配任何东西
├── baseline/       base 基线, 建议进 git 共享
└── .state/         快照 / 备份 / 日志, 本地状态, 不进 git
```

`maaowm/.gitignore` 是 MaaOWM 自己写的，开箱即用。

两条历史遗留，如果你的资源项目 `.gitignore` 里有，请处理：

- `.maaowm/` —— V3.7.9 之前状态目录放在适配包内，那时需要这行。留着无害（旧版残留仍会被忽略），删掉也行。
- `**/__mod_extras__.json` —— **务必删掉这行。** V0.7.12 之前，工作区新建的节点会被塞进适配包里这个兜底文件；它装的是真实可运行的节点定义，被 git 忽略掉就等于提交出一个 `next` 指向不存在节点的断链资源包。该兜底已取消（新建节点写回它所在的原文件），若你项目里还残留这个文件，把里面的节点搬回该在的文件后删掉它。
